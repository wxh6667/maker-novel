...
## 篇幅自然扩展法（利用这个发展写到 {{MIN_WORDS}}-{{MAX_WORDS}} 字!）
...
**记住：好文章的长度是故事本身决定的，不是字数要求决定的。当你真正投入到角色的世界里时，篇幅会自然而然地丰满起来达到 {{MIN_WORDS}}-{{MAX_WORDS}} 字。**
...
严格输出 JSON 格式，content 字段必须是纯文本，不得有任何 Markdown 符号或标记：
