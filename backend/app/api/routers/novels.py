...
async def list_novels(
    ...
    logger.debug("用户 %s 获取项目列表，共 %s 个", current_user.id, len(projects))
    return projects
...
async def get_novel(
    ...
    novel_service = NovelService(session)
    logger.debug("用户 %s 查询项目 %s", current_user.id, project_id)
...
async def get_novel_section(
    ...
    novel_service = NovelService(session)
    logger.debug("用户 %s 获取项目 %s 的 %s 区段", current_user.id, project_id, section)
...
async def converse_with_concept(
    ...
    llm_response = remove_think_tags("".join(response_chunks))
    ...
    logger.debug("项目 %s 概念对话完成，is_complete=%s", project_id, parsed.get("is_complete"))
...
async def generate_blueprint(
    ...
    project = await novel_service.ensure_project_owner(project_id, current_user.id)
    logger.debug("项目 %s 开始生成蓝图", project_id)
    ...
    # 确保消息序列符合 Claude API 要求：system 后必须是 user
    # 如果第一条是 assistant，在前面插入一个占位 user 消息
    if formatted_history and formatted_history[0]["role"] == "assistant":
        formatted_history.insert(0, {"role": "user", "content": "请开始引导我完成小说构思。"})
    ...
    # 添加明确的生成指令，告诉 LLM 现在需要生成蓝图
    generation_instruction = {
        "role": "user",
        "content": "请根据以上对话内容，直接生成完整的小说蓝图 JSON。不要进行任何对话或解释，只输出 JSON 对象。"
    }

    blueprint_chunks = []
    async for chunk in llm_service.stream_node(
        "blueprint",
        [{"role": "system", "content": system_prompt}, *formatted_history, generation_instruction],
        user_id=current_user.id,
        response_format="json_object",
    ):
        if chunk.get("content"):
            blueprint_chunks.append(chunk["content"])
    blueprint_raw = remove_think_tags("".join(blueprint_chunks))

    logger.debug(
        "项目 %s 蓝图原始响应长度: %s, 前500字符: %s",
        project_id,
        len(blueprint_raw),
        blueprint_raw[:500],
    )
...
async def patch_blueprint(
    ...
    logger.debug("项目 %s 局部更新蓝图字段：%s", project_id, list(update_data.keys()))
    return await novel_service.get_project_schema(project_id, current_user.id)
