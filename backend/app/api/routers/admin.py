...
async def read_statistics(
    ...
    logger.debug("管理员获取统计数据：小说=%s，用户=%s，请求=%s", novel_count, user_count, api_request_count)
...
async def list_users(
    ...
    logger.debug("管理员请求用户列表，共 %s 条", len(users))
...
async def list_novel_projects(
    ...
    logger.debug("管理员查看项目列表，共 %s 个", len(projects))
    return projects
...
async def get_novel_project(
    ...
    logger.debug("管理员查看项目详情：%s", project_id)
...
async def get_novel_project_section(
    ...
    logger.debug("管理员查看项目 %s 的 %s 区段", project_id, section)
...
async def get_novel_project_chapter(
    ...
    logger.debug("管理员查看项目 %s 第 %s 章详情", project_id, chapter_number)
...
async def list_prompts(
    ...
    logger.debug("管理员请求提示词列表，共 %s 条", len(prompts))
    return prompts
...
async def get_prompt(
    ...
    logger.debug("管理员获取提示词：%s", prompt_id)
    return prompt
...
async def list_update_logs(
    ...
    logger.debug("管理员查看更新日志列表，共 %s 条", len(logs))
...
async def get_daily_limit(
    ...
    logger.debug("管理员查询每日请求上限：%s", value)
...
async def list_system_configs(
    ...
    logger.debug("管理员获取系统配置，共 %s 条", len(configs))
    return configs
