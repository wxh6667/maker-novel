    ...
    if isinstance(value, dict):
        for key in _PREFERRED_CONTENT_KEYS:
            if key in value and value[key]:
                ...
                if nested:
                    return nested
        return json.dumps(value, ensure_ascii=False)  # 不调用 _clean_string 避免递归
...
class NovelService:
    ...
    async def replace_chapter_versions(self, chapter: Chapter, contents: List[str], metadata: Optional[List[Dict]] = None) -> List[ChapterVersion]:
        ...
        for index, content in enumerate(contents):
            ...
            provider_name = extra.get("_provider") if extra else None
            version = ChapterVersion(
                chapter_id=chapter.id,
                content=text_content,
                provider=provider_name,
                metadata_={"_model": extra.get("_model")} if extra and extra.get("_model") else None,
    ...
    async def select_chapter_version(self, chapter: Chapter, version_index: int) -> ChapterVersion:
        ...
        # 先设置为 SELECTING 状态，等待后续操作完成后再设置为 SUCCESSFUL
    ...
    async def add_chapter_evaluation(self, chapter: Chapter, version: Optional[ChapterVersion], feedback: str, decision: Optional[str] = None, score: Optional[float] = None) -> None:
                    ...
                    versions = [
                        {
                            "content": v.content,
                            "provider": v.provider,
                            "model": v.metadata_.get("_model") if v.metadata_ else None
                        }
