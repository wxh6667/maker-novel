...
class VectorStoreService:
    ...
    def __init__(self) -> None:
        ...
        if url and url.startswith("file:"):
            ...
            url = f"file:{resolved}"
            logger.debug("向量库使用本地文件: %s", resolved)

        try:
            logger.debug("初始化 libsql 客户端: url=%s", url)
            ...
            logger.debug("libsql 客户端初始化成功，等待建表。")
    ...
    async def _query_chunks_with_python_similarity(
        ...
        *,
        ...
        # 使用 heapq 优化 top-k 选择，避免完整排序
        heap: List[tuple[float, RetrievedChunk]] = []
        row_count = 0
        for row in self._iter_rows(result):
            row_count += 1
            ...
            if len(heap) < top_k:
                heapq.heappush(heap, (-distance, chunk))
            elif distance < -heap[0][0]:
                heapq.heapreplace(heap, (-distance, chunk))
        
        if row_count > 1000:
            logger.warning(
                "Python 回退相似度计算处理了 %d 条记录，建议安装 sqlite-vec 扩展以提升性能",
                row_count,
            )
        
        # 按距离升序返回
        result_list = [item[1] for item in sorted(heap, key=lambda x: -x[0])]
        return result_list
    ...
    async def _query_summaries_with_python_similarity(
        ...
        *,
        ...
        # 使用 heapq 优化 top-k 选择
        heap: List[tuple[float, RetrievedSummary]] = []
        row_count = 0
        for row in self._iter_rows(result):
            row_count += 1
            stored_embedding = self._from_f32_blob(row.get("embedding"))
            ...
            if len(heap) < top_k:
                heapq.heappush(heap, (-distance, summary))
            elif distance < -heap[0][0]:
                heapq.heapreplace(heap, (-distance, summary))
        
        if row_count > 500:
            logger.warning(
                "Python 回退相似度计算处理了 %d 条摘要，建议安装 sqlite-vec 扩展",
                row_count,
            )
