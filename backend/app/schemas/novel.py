...
class ChapterVersionInfo(BaseModel):
    """章节版本信息"""
    content: str
...
class Chapter(ChapterOutline):
    ...
    versions: Optional[List[ChapterVersionInfo]] = None
...
class AutoCreateRequest(BaseModel):
    """连续创作模式请求"""
    start_chapter: Optional[int] = Field(default=None, description="起始章节号，默认从下一个未完成章节开始")
    end_chapter: Optional[int] = Field(default=None, description="结束章节号，默认到大纲最后一章")
    writing_notes: Optional[str] = Field(default=None, description="全局写作指令，应用于所有章节")
    auto_stop_on_error: bool = Field(default=True, description="遇到错误是否停止")
