    ...
    <div class="flex flex-row flex-1 h-full lg:min-h-0 overflow-hidden">
      ...
      >
        <div class="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
          ...
          <button
            class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded border transition-colors duration-200"
            :class="chapters.length > 0 && !isExportingAll ? 'border-emerald-200 text-emerald-600 hover:bg-emerald-50' : 'border-slate-200 text-slate-400 cursor-not-allowed'"
            :disabled="chapters.length === 0 || isExportingAll"
            @click="exportAllChaptersAsTxt"
            title="导出全部章节为单个TXT文件"
          >
...
// 导出全部章节为单个 TXT 文件
const exportAllChaptersAsTxt = async () => {
  if (chapters.value.length === 0) return

  isExportingAll.value = true
  try {
    const allContents: string[] = []
    const novelName = props.novelTitle || '小说'

    // 按章节顺序获取所有章节内容
    for (const chapter of chapters.value) {
      let content = ''
      // 优先从缓存获取
      if (chapterCache.has(chapter.chapter_number)) {
        content = chapterCache.get(chapter.chapter_number)?.content ?? ''
      } else {
        // 从 API 获取
        try {
          const detail = props.isAdmin
            ? await AdminAPI.getNovelChapter(projectId, chapter.chapter_number)
            : await NovelAPI.getChapter(projectId, chapter.chapter_number)
          content = detail.content ?? ''
          // 缓存结果
          chapterCache.set(chapter.chapter_number, detail as ChapterDetail)
        } catch {
          content = ''
        }
      }
      ...
      const chapterTitle = titleText ? `第${chapter.chapter_number}章 ${titleText}` : `第${chapter.chapter_number}章`
      if (content) {
        allContents.push(`${'='.repeat(40)}\n${chapterTitle}\n${'='.repeat(40)}\n\n${content}\n\n`)
      }
    }
    ...
    const fullContent = `《${novelName}》\n\n` + allContents.join('')
    const safeNovelName = sanitizeFileName(novelName)
    const blob = new Blob([fullContent], { type: 'text/plain;charset=utf-8' })
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = `${safeNovelName}_全文.txt`
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
    URL.revokeObjectURL(url)
