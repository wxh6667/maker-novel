          ...
          <div class="flex items-start gap-3">
            ...
            <div class="flex-1">
              ...
              <div class="mt-2 flex items-center gap-2 text-xs text-gray-500">
                <span v-if="version.provider" class="text-indigo-600 font-medium">{{ version.provider }}</span>
                <span v-if="version.provider">•</span>
                <span>约 {{ Math.round(cleanVersionContent(version.content).length / 100) * 100 }} 字</span>
                <!-- 评分展示 -->
                <span v-if="versionScores[index] !== undefined" class="px-2 py-0.5 rounded-full text-xs font-semibold" :class="getScoreClass(versionScores[index])">
                  {{ versionScores[index] }} 分
                </span>
...
const cleanVersionContent = (content: string | any): string => {
  if (!content) return ''
  // 如果 content 不是字符串，先转换
...
// 解析评审数据，获取每个版本的评分
const versionScores = computed(() => {
  const scores: Record<number, number | null> = {}
  if (!props.selectedChapter?.evaluation) return scores

  try {
    const evalData = JSON.parse(props.selectedChapter.evaluation)

    // 新格式：versions[]
    if (evalData.versions && Array.isArray(evalData.versions)) {
      for (const v of evalData.versions) {
        if (v.version_index !== undefined && v.score !== undefined) {
          scores[v.version_index - 1] = v.score  // version_index 从1开始，转为0开始
        }
      }
    }
    // 旧格式：evaluation.versionX
    else if (evalData.evaluation) {
      for (const [key, value] of Object.entries(evalData.evaluation)) {
        const match = key.match(/version(\d+)/)
        if (match && (value as any)?.score !== undefined) {
          scores[parseInt(match[1]) - 1] = (value as any).score
        }
      }
    }
  } catch (e) {
