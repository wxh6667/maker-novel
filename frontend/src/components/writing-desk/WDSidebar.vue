          ...
          <div class="px-6 pb-6">
              ...
              >
                <div class="flex items-start gap-3">
                  ...
                  <div
                    :class="[
                      ...
                      isChapterCompleted(chapter.chapter_number)
                        ...
                        : isChapterGenerating(chapter.chapter_number) || isChapterEvaluating(chapter.chapter_number) || isChapterSelecting(chapter.chapter_number) || isChapterIngesting(chapter.chapter_number)
                  ...
                  >
                    ...
                    <svg v-else-if="isChapterGenerating(chapter.chapter_number) || isChapterSelecting(chapter.chapter_number) || isChapterIngesting(chapter.chapter_number)" class="w-4 h-4 animate-spin" fill="currentColor" viewBox="0 0 20 20">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <div class="mt-2 flex items-center gap-2 flex-wrap">
                      <!-- 评审分数 -->
                      <span
                        v-if="getChapterScore(chapter.chapter_number) !== null"
                        class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium"
                        :class="getScoreClass(getChapterScore(chapter.chapter_number)!)"
                      >
                        {{ getChapterScore(chapter.chapter_number) }} 分
                      </span>
                      ...
                      <span v-else-if="isChapterIngesting(chapter.chapter_number)" class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-cyan-100 text-cyan-800 animate-pulse">
                        入库中...
                      </span>
            ...
            <div class="mt-4 flex gap-2">
              <button
                ...
                class="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
...
const emit = defineEmits(['closeSidebar', 'selectChapter', 'generateChapter', 'editChapter', 'deleteChapter', 'generateOutline', 'autoCreate'])
...
function handleDeleteSelected() {
  ...
  if (!lastChapterNumber.value || !sortedSelection.includes(lastChapterNumber.value)) {
    globalAlert.showWarning('批量删除必须包含最后一章。', '操作提示')
    return
  }
  ...
  if (!isContinuous) {
    globalAlert.showWarning('只能删除连续的章节块。', '操作提示')
    return
  }
...
const isChapterIngesting = (chapterNumber: number) => {
  if (!props.project?.chapters) return false
  const chapter = props.project.chapters.find(ch => ch.chapter_number === chapterNumber)
  return chapter && chapter.generation_status === 'ingesting'
}

// 获取章节评审分数（返回最佳版本的分数）
const getChapterScore = (chapterNumber: number): number | null => {
  if (!props.project?.chapters) return null
  const chapter = props.project.chapters.find(ch => ch.chapter_number === chapterNumber)
  if (!chapter?.evaluation) return null

  try {
    let evalData = JSON.parse(chapter.evaluation)
    ...
    if (evalData.versions && Array.isArray(evalData.versions)) {
      const bestChoice = evalData.best_choice || 1
      const bestVersion = evalData.versions.find((v: any) => v.version_index === bestChoice)
      if (bestVersion?.score !== undefined) {
        return bestVersion.score
      }
      // 如果找不到最佳版本，返回最高分
      const scores = evalData.versions.map((v: any) => v.score).filter((s: any) => typeof s === 'number')
      if (scores.length > 0) {
    ...
    if (evalData.evaluation) {
      const versions = Object.values(evalData.evaluation) as any[]
      const scores = versions.map(v => v.score).filter(s => typeof s === 'number')
      if (scores.length > 0) {
        return Math.max(...scores)
      }
    }
